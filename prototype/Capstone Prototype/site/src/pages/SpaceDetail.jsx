...
function CapacityBar({ capacity }) {
  ...
  const label =
    capacity <= 30 ? "Intimate" :
    capacity <= 80 ? "Small–medium" :
    capacity <= 200 ? "Medium" :
    capacity <= 500 ? "Large" : "Very large";
  return (
    <div>
      <p className="text-sm text-gray-700 mb-1">
        <span className="font-semibold">Capacity: </span>{capacity} people · {label}
      </p>
      <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
        <div className="h-full bg-[#5F77A5] rounded-full" style={{ width: `${pct}%` }} />
      </div>
    </div>
  );
}
...
export default function SpaceDetail() {
  ...
  const { createdSpaces, createdEvents, deletedEventIds, editedEvents, bookmarkedEvents, toggleBookmark } = useUser();
  ...
  if (!space) {
    return (
      <main className="max-w-4xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-gray-500 mb-6">That space doesn't exist or may have been removed.</p>
        <Link
          to="/events"
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          <div className="flex-1 min-w-0 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            {space.hours && (
              <div className="px-6 pt-2 flex items-center gap-3 text-gray-700">
                <Clock size={18} className="text-[#97BFFF] shrink-0" aria-hidden="true" />
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              ...
              <div className="px-6 py-5 flex flex-col gap-3">
                ...
                {space.amenities?.length > 0 && (
                  <div>
                    <p className="text-sm font-semibold text-gray-700 mb-1.5">Amenities:</p>
        ...
        {displayEvents.length > 0 && (
          <section className="mt-14" aria-labelledby="space-events-heading">
            <h2 id="space-events-heading" className="text-xl font-bold text-gray-900 mb-5">
